...
while 1:
    # hud TEST
    panda.can_send(0x370, bytes([0x46,0xd0,0x00,0x33,0x10,0x00,0x60,0xb9]), 0)
    panda.can_send(0x373, bytes([0x42,0x00,0x00,0x00,0x6c,0x00,0x00,0xae]), 0)
    # panda.can_send(0x225, bytes([0x00,0x00,0x64,0x00,0xff,0xef,0x64,0xb6]), 0)

    # # STEERING_LKA ALMAZ (Torque)
    # panda.can_send(0x225, bytes([0x20,0x1a,0x00,0x00,0x00,0x00,0x00,0x3a]), 0)

    # # STEERING_LKA ALVEZ (Angle)
    # panda.can_send(0x225, bytes([0x00,0x00,0x64,0x00,0x80,0x23,0x64,0x6b]), 0)

    # # LKAS_HUD (BOTH ACTIVE AND STATE OK)
    # panda.can_send(0x373, bytes([0xc6,0x0c,0x01,0x00,0x6c,0x00,0x00,0x3f]), 0)

    # # LKAS_HUD (ACTIVE OK STATE NO)
    # panda.can_send(0x373, bytes([0xc2,0x00,0x00,0x00,0x6c,0x00,0x00,0x2e]), 0)

    # # LKAS_HUD (BOTH ACTIVE AND STATE OFF)
    # panda.can_send(0x373, bytes([0x80,0x00,0x00,0x00,0x6c,0x00,0x00,0xec]), 0)
    
    # GAS TEST 
    # panda.can_send(0x260, bytes([0x4f,0x5E,0x00,0x10,0x00,0x00,0x00,0xbd]), 0)
    # panda.can_send(0x260, bytes([0x0f,0xbf,0x00,0x10,0x00,0x00,0x00,0xde]), 0)
    #panda.can_send(0x260, bytes([0x0f,0xd4,0x00,0x10,0x00,0x00,0x00,0xf3]), 0)
    
    # BRAKE TEST
    # panda.can_send(0x263, bytes([0x05,0x78,0x60,0x00,0x44,0x07,0x0f,0x37]), 0)
    # panda.can_send(0x263, bytes([0x85,0x78,0x00,0x00,0x00,0x07,0x21,0x25]), 0)
    # panda.can_send(0x263, bytes([0x05,0x78,0x00,0x00,0x00,0x06,0xfd,0x80]), 0)
    # panda.can_send(0x263, bytes([0x05,0x78,0x00,0x00,0x00,0x06,0xfa,0x3d]), 0)
